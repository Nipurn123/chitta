...
let tsReady: Promise<{ Parser: any; Language: any }> | null = null
const langCache = new Map<string, unknown>()

// web-tree-sitter ≥0.25 uses named exports ({ Parser, Language }); ≤0.24 exposes a default
// export that IS the Parser class - and attaches `.Language` to it only AFTER init()
// resolves. So `Language` must be read post-init. Memoized; a failed bootstrap clears the
// memo so a later call can retry (preserving the "never crash, degrade to empty" contract).
function ts(): Promise<{ Parser: any; Language: any }> {
  return (tsReady ??= (async () => {
    const mod = (await import("web-tree-sitter")) as any
    const Parser = mod.Parser ?? mod.default
    await Parser.init()
    return { Parser, Language: mod.Language ?? Parser.Language }
  })().catch((e) => {
    tsReady = null
    throw e
  }))
...
async function loadLang(lang: string): Promise<unknown | null> {
  try {
    ...
    if (!langCache.has(lang)) {
      ...
      langCache.set(lang, await Language.load(WASM_DIR + `tree-sitter-${lang}.wasm`))
    }
...
export class CodeExtractor implements KnowledgeExtractor {
  ...
  async extractCode(code: string, lang: string, fileName = "source"): Promise<Extraction> {
    ...
    if (!tree) return { entities: [], relations: [] } // ≥0.25: parse() may return null
